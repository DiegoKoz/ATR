...
output:
  html_notebook:
    ...
    toc_float: yes
date: "7/12/2018"
subtitle: Conceptos Principales de R Base
---
...
## Tabulados y valores únicos

Dos funciones muy útiles para empezar a trabajar son `table()` y `unique()`

Para ver cómo funcionan, utilizamos el dataframe __individual_t117__


`unique()` nos devuelve un vector con los valores únicos, es decir sin duplicar. Por ejemplo

```{r}
unique(individual_t117$REGION)
```

`Table` sirve para calcular frecuencias relativas
```{r}
table(individual_t117$REGION)
```
Esto nos da la cantidad de casos por región.

Para hacer una tabla bi-variada, simplemente le pasamos dos vectores

```{r}
tabla <- table(individual_t117$REGION, individual_t117$CH04)
tabla
```


Si queremos que la tabla de frecuencias sea en porcentaje, podemos utilizar el comando `prop.table()` sobre el resultado anterior
...
Vamos a instalar las librerías que necesitamos para el curso: 

```
install.packages(c("tidyverse","openxlsx",'statar','reldist', 'ggthemes', 'ggrepel', 'scales', 'ggridges','questionr', 'alluvial','httr', 'stringr','xlsx','haven','readxl','knitr','ggalt','kableExtra','formattable'))
```
